...
Tickets that `to-tickets` produces are only agent-ready after they meet the repository's Definition Of Ready. That policy defines required traceability, acceptance criteria, non-goals, verification, and unresolved-decision fields. Don't run [triage](https://aihero.dev/skills-triage) over complete generated tickets. Triage is for work that arrived from someone else.

## Prerequisites

`to-tickets` publishes into a tracker, so [setup-matt-pocock-skills](https://aihero.dev/skills-setup-matt-pocock-skills) must have configured one for this repo, along with the triage-label vocabulary and delivery workflow. Either kind works: a real tracker like GitHub or Linear, or local markdown files under `.scratch/`, which is supported out of the box.
...
The skill stops at the artifact, and there is no auto-dispatch mode. Dispatch is manual: look at the board, count the tickets with no open blockers, and open that many agent sessions. One ticket per fresh context, cleared between them. [implement](https://aihero.dev/skills-implement) then follows the repository's delivery workflow to create the branch and change request, record evidence, and close the issue when the policy authorizes a merge.
